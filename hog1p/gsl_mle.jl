...
function logl_gsl(params)
    global prob = remake(prob,p=params)
    sol = solve(prob, KenCarp4(), saveat=tt)
    simulate_fsp!(data_sim, sol)
   
    means = mean.(data_sim[i,:,:][:] for i in 1:length(tt) )
    stds = std.(data_sim[i,:,:][:] for i in 1:length(tt) )
    
    xx = 0:150
    ret = sum.(logpdf(Normal(means[i], stds[i]), xx) .* data_obs[i,1,:] for i in 1:length(tt) )
